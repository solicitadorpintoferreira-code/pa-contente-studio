...
async function callAPI(system, messages, maxT = 1500, useSearch = false) {
...
function trackCost(c) {
  ...
  costs.total += c; PA.set('costs', costs);
...
function updateCostDisplay() {
  ...
  set('cSess', window._sessC||0); set('cMonth', costs.month[month]||0); set('cTotal', costs.total||0);
}

function getSys() {
  return `És o assistente pessoal e estratégico de Patrício Ferreira. Responde SEMPRE em português de Portugal.
IDENTIDADE: Solicitador e Partner na P&A Legal. Especialista em arquitetura empresarial, holdings, pactos de sócios, SPVs e proteção de ativos.
VOZ: Editorial, provocatório, calmo, autoritário.
NUNCA: bullet points, emojis, em-dashes, linguagem de coaching.
PÚBLICO: Empreendedores com empresas entre 500k e 5M de faturação.`;
}

function toast(msg, type='') {
  let el = document.getElementById('toast');
  if (!el) { el = document.createElement('div'); el.id='toast'; document.body.appendChild(el); }
  el.textContent = msg; el.className = 'toast on' + (type?' toast-'+type:'');
  clearTimeout(window._toastT); window._toastT = setTimeout(() => el.classList.remove('on'), 2600);
}
...
function buildNav(current) {
  ...
  const pages = {'profissional.html':'Profissional','pessoal.html':'Pessoal','livro.html':'O Livro'};
