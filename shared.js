...
// ── RSS NEWS — multiple proxy fallbacks ──
async function fetchRSS(query) {
  const topics = query || PA.get('themes', ['holdings portugal']).join(' OR ');
  const encoded = encodeURIComponent(topics + ' Portugal');
  const rssUrl = `https://news.google.com/rss/search?q=${encoded}&hl=pt-PT&gl=PT&ceid=PT:pt`;
  ...
  for (const proxyUrl of proxies) {
    try {
      ...
      const xmlText = raw.contents || raw;
